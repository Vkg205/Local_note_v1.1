...
public sealed class MainViewModel : ObservableObject
{
    ...
    private bool _suppressAutoNavigationLoads;
    ...
    public Func<Task>? BeforeVaultSwitchAsync { get; set; }
    ...
    public Notebook? SelectedNotebook
    {
        ...
        set
        {
            ...
            RefreshCanExecute();
            if (!_suppressAutoNavigationLoads)
                _ = RunNavigationLoadAsync(() => LoadSectionsAsync(value));
        }
    }
    ...
    public Section? SelectedSection
    {
        ...
        set
        {
            ...
            RefreshCanExecute();
            if (!_suppressAutoNavigationLoads)
                _ = RunNavigationLoadAsync(() => LoadPagesAsync(value));
        }
    }
    ...
    public async Task RenameSelectedPageInlineAsync(string title)
    {
        ...
        // Renaming used to reload the entire page collection. That temporarily cleared
        // SelectedPage, rebound the canvas to null, and could race with Enter creating
        // the first text block on a newly created page. Update the navigation item in
        // place without changing the active page/canvas.
        var current = SelectedPage;
        await _pageRepository.RenameAsync(current.Id, title);
        var updated = new Page
        {
            Id = current.Id, SectionId = current.SectionId, ParentPageId = current.ParentPageId,
            Title = title, IndentLevel = current.IndentLevel, SortOrder = current.SortOrder,
            IsPinned = current.IsPinned, IsDeleted = current.IsDeleted, PaperStyle = current.PaperStyle, LocalVersion = current.LocalVersion + 1,
            CreatedAt = current.CreatedAt, UpdatedAt = DateTimeOffset.UtcNow
        };
    ...
    public async Task UpdateSelectedPagePaperStyleAsync(string paperStyle)
    {
        if (SelectedPage is null || _pageRepository is null) return;
        paperStyle = paperStyle is "Grid" or "Lines" or "Dots" ? paperStyle : "Blank";
        if (string.Equals(SelectedPage.PaperStyle, paperStyle, StringComparison.Ordinal)) return;

        var current = SelectedPage;
        await _pageRepository.UpdatePaperStyleAsync(current.Id, paperStyle);
        var updated = new Page
        {
            Id = current.Id, SectionId = current.SectionId, ParentPageId = current.ParentPageId,
            Title = current.Title, IndentLevel = current.IndentLevel, SortOrder = current.SortOrder,
            IsPinned = current.IsPinned, IsDeleted = current.IsDeleted, PaperStyle = paperStyle, LocalVersion = current.LocalVersion + 1,
            CreatedAt = current.CreatedAt, UpdatedAt = DateTimeOffset.UtcNow
        };
        var index = Pages.ToList().FindIndex(x => x.Id == current.Id);
        if (index >= 0) Pages[index] = updated;
        if (SelectedPage?.Id == current.Id)
        {
            _selectedPage = updated;
            Raise(nameof(SelectedPage));
        }
    ...
    public async Task ChangeSelectedSectionColorAsync(string color)
    {
        ...
        if (index >= 0)
        {
            ...
            // Color-only changes must not reload pages/canvas.
    ...
    public async Task NavigateToPageAsync(string pageId)
    {
        ...
        // Search/result navigation is a single transaction. Suppress property-setter
        // background loads for its entire lifetime; otherwise LoadNotebooksAsync,
        // SelectedNotebook and SelectedSection can start overlapping section/page
        // queries and reset the selection after the target page has already opened.
        _suppressAutoNavigationLoads = true;
        try
        {
            ...
            if (notebook is null)
            {
                ...
                notebook = Notebooks.FirstOrDefault(x => x.Id == context.Value.NotebookId);
            }
            if (notebook is null) return;

            SelectedNotebook = notebook;
            await LoadSectionsAsync(notebook);

            var section = Sections.FirstOrDefault(x => x.Id == context.Value.SectionId);
            if (section is null) return;

            SelectedSection = section;
            await LoadPagesAsync(section);
            SelectedPage = Pages.FirstOrDefault(x => x.Id == pageId);
        }
        finally
        {
            _suppressAutoNavigationLoads = false;
        }
    }
    ...
    private async Task CreateVaultAsync()
    {
        ...
        {
            if (BeforeVaultSwitchAsync is not null) await BeforeVaultSwitchAsync();
            ...
            try
            {
                ...
                await EnsureStarterContentAsync();
                await LoadNotebooksAsync();
                await _settings.SaveLastVaultAsync(vault.RootPath);
                StatusText = "本地仓库已创建";
            }
            catch
            {
    ...
    private async Task LoadVaultAsync(string path)
    {
        if (BeforeVaultSwitchAsync is not null) await BeforeVaultSwitchAsync();
        await CloseCurrentVaultAsync();
        VaultWriteLock? newLock = null;
        try
        {
            newLock = VaultWriteLock.Acquire(path);
        ...
        {
            if (ReferenceEquals(_writeLock, newLock)) _writeLock = null;
            newLock?.Dispose();
            try { await CloseCurrentVaultAsync(); } catch { }
            throw;
        }
    }
    ...
    private async Task CloseCurrentVaultAsync()
    {
        ...
        finally
        {
            ...
            _selectedNotebook = null; Raise(nameof(SelectedNotebook));
            _selectedSection = null; Raise(nameof(SelectedSection));
            _selectedPage = null; Raise(nameof(SelectedPage));
            Raise(nameof(VaultSummary));
            RefreshCanExecute();
            ActivePageChanged?.Invoke(this, EventArgs.Empty);
        }
    }
    ...
    {
        var loadVersion = Interlocked.Increment(ref _sectionLoadVersion);
        Sections.Clear(); Pages.Clear(); _selectedSection = null; Raise(nameof(SelectedSection)); _selectedPage = null; Raise(nameof(SelectedPage)); RefreshCanExecute(); ActivePageChanged?.Invoke(this, EventArgs.Empty);
        if (notebook is null || _sectionRepository is null) return;
        var notebookId = notebook.Id;
        var items = await _sectionRepository.GetActiveAsync(notebookId);
        if (loadVersion != _sectionLoadVersion || _selectedNotebook?.Id != notebookId) return;
        foreach (var item in items) Sections.Add(item);
        var desired = _restoreSectionId ?? (_lastSectionByNotebook.TryGetValue(notebookId, out var remembered) ? remembered : null);
    ...
    {
        var loadVersion = Interlocked.Increment(ref _pageLoadVersion);
        Pages.Clear(); _selectedPage = null; Raise(nameof(SelectedPage)); RefreshCanExecute(); ActivePageChanged?.Invoke(this, EventArgs.Empty);
        if (section is null || _pageRepository is null) return;
        var sectionId = section.Id;
        var items = await _pageRepository.GetActiveAsync(sectionId);
        if (loadVersion != _pageLoadVersion || _selectedSection?.Id != sectionId) return;
        foreach (var item in items) Pages.Add(item);
        var desired = _restorePageId ?? (_lastPageBySection.TryGetValue(sectionId, out var remembered) ? remembered : null);
        _restorePageId = null;
    ...
    private async Task RunNavigationLoadAsync(Func<Task> action)
    {
        try { await action(); }
        catch (Exception ex) { StatusText = $"导航加载失败：{ex.Message}"; }
    }
